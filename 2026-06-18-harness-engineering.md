...
> ⚠️ 以下为方向性指引；**链接请以各家官方最新为准**。若需要，我可以用 deep-research 把这一节拉成"带最新链接 + 要点摘要"的核对版本（见文末）。

**Anthropic**
- 《Building Effective Agents》（workflow vs agent 的取舍、何时该上 agent）—— 本节范式②的来源。
- Claude Docs：Tool use（工具/函数调用）、Agents、Evaluating prompts（evals）、Prompt engineering。
- 工程博客：context engineering 专题、multi-agent 系统复盘（如多 agent 研究系统的经验教训）。

**OpenAI**
- 《A Practical Guide to Building Agents》（实践指南，覆盖 loop / guardrails / 工具设计）。
- Evals 文档（如何建评测集、自动判分、回归）。
- Agents SDK / function calling（工具即 API 的官方实现）。
